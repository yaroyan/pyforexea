...
class Ticker(object):
    ...
    # 2020-01-01 11:22:37
    # 2020-01-01 11:22:35 5S
    # 2020-01-01 11:22:00 1M
    # 2020-01-01 11:00:00 1H
    def truncate_date_time(self, duration):
        ...
        if duration == constants.DURATION_5S:
            ...
            logger.warning(
                'action=truncate_date_time error=no_datetime_format')
            return None
...
class APIClient(object):
    ...
    def get_candle_volume(self, count=1, granularity=constants.TRADE_MAP[settings.trade_duration]['granularity']) -> int:
        params = {
            'count': count,
            'granularity': granularity,
        }
        req = instruments.InstrumentsCandles(
            instrument=settings.product_code, params=params)
        try:
            resp = self.client.request(req)
        except V20Error as e:
    ...
    def get_realtime_ticker(self, callback):
        req = PricingStream(accountID=self.account_id, params={
            'instrument': settings.product_code,
        })
        try:
    ...
    def get_order(self, order_id) -> Order:
        ...
        order = Order(
            ...
            side=constants.BUY if float(
                resp['order']['units']) > 0 else constants.SELL,
    ...
    def trade_details(self, trade_id) -> Trade:
        ...
        trade = Trade(
            trade_id=trade_id,
            side=constants.BUY if float(
                resp['trade']['currentUnits']) > 0 else constants.SELL,
    ...
    def get_open_trade(self) -> list:
        ...
        for trade in resp['trades']:
            trades_list.insert(0, Trade(
                trade_id=trade['id'],
                side=constants.BUY if float(
                    trade['currentUnits']) > 0 else constants.SELL,
    ...
    def trade_close(self, trade_id) -> Trade:
        ...
        trade = Trade(
            trade_id=trade_id,
            side=constants.BUY if float(
                resp['orderFillTransaction']['units']) > 0 else constants.SELL,
